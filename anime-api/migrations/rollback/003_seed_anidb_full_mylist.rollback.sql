-- Rollback for 003_seed_anidb_full_mylist.sql
-- Removes only rows seeded by that migration via (title, year_from) identity.

BEGIN;

DELETE FROM anime
WHERE (title, year_from) IN (
  ('07-Ghost', 2009),
  ('Aho-Girl', 2017),
  ('Aika Zero', 2009),
  ('Ajin: Demi-Human', 2016),
  ('Aldnoah.Zero', 2014),
  ('Aldnoah.Zero (2015)', 2015),
  ('Amagami SS', 2010),
  ('Amagami SS +Plus', 2012),
  ('Angel Beats!', 2010),
  ('Anohana: The Flower We Saw That Day', 2011),
  ('Waiting in the Summer', 2012),
  ('B Gata H Kei: Yamada`s First Time', 2010),
  ('Baccano!', 2007),
  ('Baka and Test: Summon the Beasts', 2010),
  ('Baka and Test: Summon the Beasts (2)', 2011),
  ('Baka and Test: Summon the Beasts - OVA', 2011),
  ('The Boy and the Beast', 2015),
  ('Bakemonogatari', 2009),
  ('Beyblade', 2001),
  ('Black Rock Shooter', 2012),
  ('Bleach', 2004),
  ('Bleach: Thousand-Year Blood War', 2022),
  ('Bleach: Thousand-Year Blood War - The Calamity', 2026),
  ('Bleach: Thousand-Year Blood War - The Separation', 2023),
  ('Bleach: Thousand-Year Blood War - The Conflict', 2024),
  ('My Hero Academia', 2016),
  ('My Hero Academia OVA', 2016),
  ('My Hero Academia Season 2', 2017),
  ('My Hero Academia Season 3', 2018),
  ('My Hero Academia Season 4', 2019),
  ('My Hero Academia Season 5', 2021),
  ('My Hero Academia Season 6', 2022),
  ('My Hero Academia Season 7', 2024),
  ('My Hero Academia Final Season', 2025),
  ('My Hero Academia: Heroes Rising', 2019),
  ('My Hero Academia: Two Heroes', 2018),
  ('My Hero Academia: Make It! Do-or-Die Survival Training', 2020),
  ('Haganai', 2011),
  ('Haganai Next', 2013),
  ('Boruto: Naruto the Movie', 2015),
  ('Brotherhood: Final Fantasy XV', 2016),
  ('C3', 2011),
  ('Carnival Phantasm', 2011),
  ('Carnival Phantasm: EX Season', 2011),
  ('Chaos;HEAd', 2008),
  ('Love, Chunibyo & Other Delusions!', 2012),
  ('Love, Chunibyo and Other Delusions! Heart Throb', 2014),
  ('Clannad', 2007),
  ('Clannad: After Story', 2008),
  ('Code Geass: Lelouch of the Rebellion', 2006),
  ('Code Geass: Lelouch of the Rebellion R2', 2008),
  ('Date a Live', 2013),
  ('Date a Live II', 2014),
  ('Date a Live: Date to Date', 2013),
  ('Deadman Wonderland', 2011),
  ('Deadman Wonderland: The Red Knife Wielder', 2011),
  ('Death Note: Re-Light - Visions of a God', 2007),
  ('Death Note: Re-Light 2 - L`s Successors', 2008),
  ('Den-noh Coil', 2007),
  ('Devil May Cry', 2007),
  ('Digimon: Digital Monsters', 1999),
  ('Digimon Adventure 02', 2000),
  ('Dragon Ball', 1986),
  ('Dragon Ball GT', 1996),
  ('Dragon Ball Z Kai', 2009),
  ('Dragon Ball Z Kai: The Final Chapters', 2014),
  ('Dragon Ball Super', 2015),
  ('Dragon Ball Z', 1989),
  ('Dragon Ball Z: The Dead Zone', 1989),
  ('Dragon Ball Z: Bardock - The Father of Goku', 1990),
  ('Dragon Ball Z: The Tree of Might', 1990),
  ('Dragon Ball Z: Fusion Reborn', 1995),
  ('Dragon Ball Z: The Return of Cooler', 1992),
  ('Dragon Ball Z: Bojack Unbound', 1993),
  ('Dragon Ball Z: Battle of Gods', 2013),
  ('Dragon Ball Z: Broly - Second Coming', 1994),
  ('Dragon Ball Z: The World`s Strongest', 1990),
  ('Dragon Ball Z: Super Android 13!', 1992),
  ('Dragon Ball Z: Broly, the Legendary Super Saiyan', 1993),
  ('Dragon Ball Z: Wrath of the Dragon', 1995),
  ('Dragon Ball Z: Lord Slug', 1991),
  ('Dragon Ball Z: Bio-Broly', 1994),
  ('Dragon Ball Z: Cooler`s Revenge', 1991),
  ('Dragon Ball Z: The History of Trunks', 1993),
  ('Durarara!!', 2010),
  ('InuYasha the Movie 4: Fire on the Mystic Island', 2004),
  ('InuYasha the Movie 2: The Castle Beyond the Looking Glass', 2002),
  ('InuYasha the Movie 3: Swords of an Honorable Ruler', 2003),
  ('InuYasha the Movie: Affections Touching Across Time', 2001),
  ('K-On! The Movie', 2011),
  ('Elementhunters', 2009),
  ('Ergo Proxy', 2006),
  ('Final Fantasy VII: Advent Children', 2005),
  ('Final Fantasy: The Spirits Within', 2001),
  ('Full Metal Panic!', 2002),
  ('Full Metal Panic! The Second Raid', 2005),
  ('Full Metal Panic? Fumoffu', 2003),
  ('Gabriel Dropout', 2017),
  ('High School of the Dead', 2010),
  ('High School of the Dead: Drifters of the Dead', 2011),
  ('AnoHana: The Flower We Saw That Day: The Movie', 2013),
  ('Bleach the Movie 3: Fade to Black', 2008),
  ('Bleach the Movie 4: Hell Verse', 2010),
  ('Bleach the Movie: Memories of Nobody', 2006),
  ('Bleach the Movie 2: The Diamond Dust Rebellion', 2007),
  ('Fullmetal Alchemist the Movie: Conqueror of Shamballa', 2005),
  ('The Garden of Sinners', 2007),
  ('Demon Slayer: Infinity Castle', 2025),
  ('Made in Abyss: Dawn of the Deep Soul', 2020),
  ('Puella Magi Madoka Magica the Movie', 2012),
  ('Puella Magi Madoka Magica the Movie - Walpurgisnacht: Rising', 2026),
  ('The Seven Deadly Sins: Cursed by Light', 2021),
  ('Naruto Shippuden The Movie', 2007),
  ('Naruto Shippuden the Movie: The Will of Fire', 2009),
  ('Naruto Shippuden the Movie: Bonds', 2008),
  ('Naruto Shippuden the Movie: The Lost Tower', 2010),
  ('Gekijouban Naruto Soyokazeden: Naruto to Mashin to Mitsu no Onegai Datte ba yo!!', 2010),
  ('Naruto Shippuden the Movie: Blood Prison', 2011),
  ('Naruto the Movie 2: Legend of the Stone of Gelel', 2005),
  ('Naruto the Movie: Ninja Clash in the Land of Snow', 2004),
  ('Naruto the Movie 3: Guardians of the Crescent Moon Kingdom', 2006),
  ('Pokemon 3: The Movie - Spell of the Unown', 2000),
  ('Pokemon The Movie 2000: The Power of One', 1999),
  ('Pokemon: The First Movie - Mewtwo Strikes Back', 1998),
  ('Shakugan no Shana: The Movie', 2007),
  ('Made in Abyss (Movies)', 2019),
  ('Spy x Family Code: White', 2023),
  ('Sword Art Online the Movie: Ordinal Scale', 2017),
  ('Sword Art Online the Movie: Progressive - Aria of a Starless Night', 2021),
  ('Sword Art Online the Movie: Progressive - Scherzo of Deep Night', 2022),
  ('A Certain Magical Index the Movie: The Miracle of Endymion', 2013),
  ('Girls und Panzer', 2012),
  ('Brynhildr in the Darkness', 2014),
  ('Green Green (2003)', 2003),
  ('Fullmetal Alchemist', 2003),
  ('Fullmetal Alchemist: Brotherhood', 2009),
  ('Fullmetal Alchemist: The Sacred Star of Milos', 2011),
  ('Haibane Renmei', 2002),
  ('Fighting Spirit: Champion Road', 2003),
  ('Fighting Spirit', 2000),
  ('Fighting Spirit: Mashiba vs. Kimura', 2003),
  ('Fighting Spirit: New Challenger', 2009),
  ('Fighting Spirit: Rising', 2013),
  ('Hakuoki: Demon of the Fleeting Blossom - Record of the Jade Blood', 2010),
  ('Hakuoki: Demon of the Fleeting Blossom', 2010),
  ('Aria the Scarlet Ammo', 2011),
  ('Eden of the East: The King of Eden', 2009),
  ('Honoo no Chuunin Shiken! Naruto vs Konohamaru!!', 2011),
  ('Grave of the Fireflies', 1988),
  ('Howl`s Moving Castle', 2004),
  ('Demon King Daimao', 2010),
  ('Strawberry 100%', 2005),
  ('InuYasha', 2000),
  ('InuYasha: The Final Act', 2009),
  ('ItaKiss', 2008),
  ('K-On!', 2009),
  ('K-On! Season 2', 2010),
  ('Maid Sama!', 2010),
  ('Chillin` in My 30s After Getting Fired from the Demon King`s Army', 2023),
  ('The World God Only Knows', 2010),
  ('The World God Only Knows II', 2011),
  ('The World God Only Knows OVA: 4 Girls and an Idol', 2011),
  ('The World God Only Knows OVA: Magical Star Kanon 100%', 2013),
  ('The World God Only Knows: Goddesses', 2013),
  ('The World God Only Knows OVA: Tenri Arc', 2012),
  ('Heaven`s Memo Pad', 2011),
  ('Kill la Kill', 2013),
  ('Kill Me Baby', 2012),
  ('Demon Slayer', 2019),
  ('Demon Slayer: Hashira Training Arc', 2024),
  ('Demon Slayer: Swordsmith Village Arc', 2023),
  ('Demon Slayer: Mugen Train Arc', 2021),
  ('Demon Slayer: Entertainment District Arc', 2021),
  ('The 100 Girlfriends Who Really, Really, Really, Really, Really Love You', 2023),
  ('Your Name.', 2016),
  ('Kingsglaive: Final Fantasy XV', 2016),
  ('Kiss x Sis', 2008),
  ('Kiss x Sis (2010)', 2010),
  ('Konosuba: An Explosion on this Wonderful World!', 2023),
  ('Konosuba: God`s Blessing on This Wonderful World!', 2016),
  ('Konosuba: God`s Blessing on This Wonderful World! 2', 2017),
  ('Konosuba: God`s Blessing on This Wonderful World! 2 God`s Blessing on These Wonderful Works of Art!', 2017),
  ('Konosuba: God`s Blessing on This Wonderful World! 3', 2024),
  ('Konosuba: God`s Blessing on This Wonderful World! God`s Blessing on This Wonderful Choker!', 2016),
  ('Konosuba: God`s Blessing on This Wonderful World! - Legend of Crimson', 2019),
  ('The Troubled Life of Miss Kotoura', 2013),
  ('Last Order: Final Fantasy VII', 2005),
  ('Love Hina', 2000),
  ('Love Hina Again', 2002),
  ('Love Hina Christmas Movie', 2000),
  ('Love Hina Spring Movie', 2001),
  ('Love Lab', 2013),
  ('Lucky Star', 2007),
  ('Lucky Star OVA', 2008),
  ('Made in Abyss', 2017),
  ('Made in Abyss (2026)', 2026),
  ('Made in Abyss: The Golden City of the Scorching Sun', 2022),
  ('Magia Record: Puella Magi Madoka Magica Side Story', 2020),
  ('Magia Record: Puella Magi Madoka Magica Side Story 2nd Season - The Eve of Awakening', 2021),
  ('Magia Record: Puella Magi Madoka Magica Side Story Final Season - Dawn of a Shallow Dream', 2022),
  ('Negima!', 2005),
  ('Puella Magi Madoka Magica', 2011),
  ('Magical Girl Site', 2018),
  ('Maken-ki! Battling Venus', 2011),
  ('Penguindrum', 2011),
  ('Mayo Chiki!', 2011),
  ('The Future Diary', 2011),
  ('Mirai Nikki Redial: Data Ikou', 2013),
  ('Problem Children Are Coming from Another World, Aren`t They?', 2013),
  ('Problem Children Are Coming from Another World, Aren`t They?: The Hot Spring Fun Trip', 2013),
  ('Motto To Loveru', 2010),
  ('Mushoku Tensei: Jobless Reincarnation Season 2', 2023),
  ('Mushoku Tensei: Jobless Reincarnation Season 2 (2024)', 2024),
  ('Mushoku Tensei: Jobless Reincarnation Season 3', 2026),
  ('Mushoku Tensei: Jobless Reincarnation', 2021),
  ('Mushoku Tensei: Jobless Reincarnation (2021)', 2021),
  ('A Lull in the Sea', 2013),
  ('The Seven Deadly Sins', 2014),
  ('The Seven Deadly Sins: Dragon`s Judgement', 2021),
  ('The Seven Deadly Sins: Revival of the Commandments', 2018),
  ('The Seven Deadly Sins: Imperial Wrath of the Gods', 2019),
  ('The Seven Deadly Sins: Signs of Holy War', 2016),
  ('Naruto', 2002),
  ('Naruto Narutimate Hero 3: Tsui ni Gekitotsu! Jounin tai Genin!! Musabetsu Dairansen Taikai Kaisai!!', 2005),
  ('Naruto Shippuden', 2007),
  ('Naruto x UT', 2011),
  ('Naruto: The Cross Roads', 2009),
  ('Negima!? Magister Negi Magi', 2006),
  ('New Game!', 2016),
  ('New Game!!', 2017),
  ('Nichijou: My Ordinary Life', 2011),
  ('Ninja Nonsense: The Legend of Shinobu', 2004),
  ('Nisemonogatari', 2012),
  ('No Game No Life Zero', 2017),
  ('Nobunagun', 2013),
  ('Noragami', 2014),
  ('Noragami Aragoto OAD', 2015),
  ('Noragami OAD', 2014),
  ('Nyan Koi!', 2009),
  ('The Ambition of Oda Nobuna', 2012),
  ('One-Punch Man', 2015),
  ('Because I Don`t Like My Big Brother at All!', 2011),
  ('Oreimo', 2010),
  ('Oreimo 2', 2013),
  ('My Mental Choices Are Completely Interfering with My School Romantic Comedy', 2013),
  ('Oruchuban Ebichu', 1999),
  ('Pokemon', 1997),
  ('Pokemon: Mewtwo Returns', 2000),
  ('Pokemon Origins', 2013),
  ('Rio: Rainbow Gate!', 2011),
  ('Road to Ninja: Naruto the Movie', 2012),
  ('Saki: Zenkoku Hen', 2014),
  ('The Pet Girl of Sakurasou', 2012),
  ('School Rumble', 2004),
  ('School Rumble: 2nd Semester', 2006),
  ('School Rumble San Gakki', 2008),
  ('School Rumble: Extra Class', 2005),
  ('Student Council`s Discretion', 2009),
  ('Spirited Away', 2001),
  ('Senran Kagura', 2013),
  ('Servant x Service', 2013),
  ('My Bride Is a Mermaid', 2007),
  ('My Bride Is a Mermaid OVA', 2008),
  ('Shakugan no Shana', 2005),
  ('Shakugan no Shana Final', 2011),
  ('Shakugan no Shana S', 2009),
  ('Shakugan no Shana Second', 2007),
  ('Corpse Princess: Aka', 2008),
  ('Corpse Princess: Kuro', 2009),
  ('Attack on Titan', 2013),
  ('Shingeki no Kyojin OAD', 2013),
  ('Attack on Titan Season 2', 2017),
  ('Attack on Titan Season 3', 2018),
  ('Attack on Titan Season 3 (2019)', 2019),
  ('Attack on Titan The Final Season', 2020),
  ('Attack on Titan The Final Season (2022)', 2022),
  ('Attack on Titan The Final Season (2023)', 2023),
  ('Squid Girl', 2010),
  ('Shinryaku!? Ika Musume', 2011),
  ('World`s End Harem', 2021),
  ('Special A', 2008),
  ('Spy x Family', 2022),
  ('Spy x Family (2022)', 2022),
  ('Spy x Family (2023)', 2023),
  ('Spy x Family (2025)', 2025),
  ('Steins;Gate', 2011),
  ('Steins;Gate 0', 2018),
  ('The Disappearance of Haruhi Suzumiya', 2010),
  ('The Melancholy of Haruhi Suzumiya', 2006),
  ('The Melancholy of Haruhi Suzumiya (2009)', 2009),
  ('Sword Art Online', 2012),
  ('Sword Art Online Alternative: Gun Gale Online', 2018),
  ('Sword Art Online II', 2014),
  ('Sword Art Online Alicization', 2018),
  ('Sword Art Online: Alicization - War of Underworld', 2019),
  ('Sword Art Online: Alicization - War of Underworld (2020)', 2020),
  ('Sword Art Online: Extra Edition', 2013),
  ('Love, Chunibyo and Other Delusions! Rikka Version', 2013),
  ('Tekken: Blood Vengeance', 2011),
  ('The Last: Naruto the Movie', 2014),
  ('To Loveru', 2008),
  ('To Loveru: Darkness', 2012),
  ('To Loveru: Darkness 2', 2015),
  ('To Love-Ru: Trouble - Darkness OAD', 2012),
  ('To Love-Ru: Trouble - Multiplication - Mae kara Ushiro kara', 2017),
  ('To Love-Ru: Trouble (2009)', 2009),
  ('A Certain Scientific Accelerator', 2019),
  ('A Certain Scientific Railgun', 2009),
  ('A Certain Scientific Railgun S', 2013),
  ('Toaru Kagaku no Railgun S: Daiji na Koto wa Zenbu Sentou ni Osowatta', 2014),
  ('A Certain Scientific Railgun T', 2020),
  ('Toaru Kagaku no Railgun: Entenka no Satsuei Model mo Raku ja Arimasen wa ne.', 2010),
  ('Toaru Kagaku no Railgun: Misaka-san wa Ima Chuumoku no Mato Desu kara', 2010),
  ('A Certain Magical Index', 2008),
  ('A Certain Magical Index II', 2010),
  ('A Certain Magical Index III', 2018),
  ('Tokyo Ghoul', 2014),
  ('Tokyo Ghoul Root A', 2015),
  ('Tokyo Godfathers', 2003),
  ('Tokyo Majin - Second Act', 2007),
  ('Tokyo Majin', 2007),
  ('Tomo-chan Is a Girl!', 2023),
  ('My Neighbor Totoro', 1988),
  ('Hamtaro', 2000),
  ('True Tears', 2008),
  ('Tsugu Tsugumomo', 2020),
  ('Tsugumomo', 2017),
  ('Tsugumomo (2020)', 2020),
  ('Bunny Drop', 2011),
  ('Watamote: No Matter How I Look at It, It`s You Guys` Fault I`m Not Popular!', 2013),
  ('Wizard Barristers', 2014),
  ('Wagnaria!!', 2010),
  ('Wagnaria!!3', 2015),
  ('Wagnaria!!2', 2011),
  ('My Teen Romantic Comedy: SNAFU', 2013),
  ('Yozakura Quartet', 2008),
  ('Yuu Gi Ou', 1998),
  ('Yuu Gi Ou (1999)', 1999),
  ('Yu-Gi-Oh!', 2000),
  ('Yu-Gi-Oh!: The Movie', 2004),
  ('Yusibu: I Couldn`t Become a Hero, So I Reluctantly Decided to Get a Job', 2013),
  ('Terror in Resonance', 2014)
);

COMMIT;

